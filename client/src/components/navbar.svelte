...
<nav class="
  ...
  <ul class="navbar-nav hidden sm:flex pl-0 list-style-none mr-auto">
    ...
    <li class="nav-item p-2">
      <a class="nav-link text-gray-500 hover:text-gray-700 focus:text-gray-700 p-0" href={url}>{name}</a>
    </li>
	{/each}
...
<div class:lefttog="{togglemenu === false}" class="bg-blue-300 transition-all sm:hidden duration-300 z-2 mx-auto my-auto w-screen h-screen fixed flex items-stretch justify-center">
	<ul class="self-center">
		{#each links as {name, url}}
		<li class=" text-3xl leading-8 mb-2 mt-2 py-1 px-5">
		  <a class="" on:click="{toggler}" href={url}>{name}</a>
		</li>
		{/each}
	  </ul>
  </div>
